...
public class Space extends JPanel implements Runnable{
	...
	public ArrayList<SpaceOBJ> playerFleet=new ArrayList<SpaceOBJ>();
	...
	private SpaceOBJ collide(SpaceOBJ obj){
		for(int i=0;i<objects.size();i++){
			if(obj!=objects.get(i)){
				if(obj.collision(objects.get(i))){return objects.get(i);}
			}
		}
		return null;
	}
	...
	private void shot(SpaceOBJ obj, Bullet bullet){
		if(obj.collision(bullet) && obj.team()!=bullet.team()){
	...
	public void run() {
		...
		while(true){
			...
			for(SpaceOBJ obj :objects){
					...
					if(obj.getTarget()!=null && !(obj.getTarget().team().equals(obj.team())) && getDistance(obj, obj.getTarget()) <= ((AttackShip)obj).getMaxDistance()){
						if(obj.ammoCount()>0 && obj.fireAgain()){
							try{
								...
								bullet.setTeam(obj.team());
								double r=getDistance(obj,obj.getTarget());
								double di=Math.pow(Math.pow(obj.getXLim(),2)+Math.pow(obj.getYLim(), 2), 0.5);
								int x=(int) (obj.getX()+obj.getXLim()/2+50*(obj.getTarget().getX()+obj.getTarget().getXLim()-obj.getX()-obj.getXLim())/r);
								int y=(int) (obj.getY()+obj.getYLim()/2+50*(obj.getTarget().getY()+obj.getTarget().getYLim()-obj.getY()-obj.getYLim())/r);
								bullet.setX(x);
								bullet.setY(y);
				...
				obj.recharge();
				if(obj.team()!=Color.red && obj.team()!=Color.white){
					playerFleet.add(obj);
				}
			}
			...
			repaint();
			ui.update();
			ai.updatePlayerFleet(playerFleet);
			try {
				Thread.sleep(100);
